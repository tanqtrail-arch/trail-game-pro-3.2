...
router.get('/:tenantSlug/rankings', optionalAuth, (req, res) => {
  ...
  if (gameId) {
    ...
    // グローバルランキング（ALT合計順）— 期間フィルタ対応
    let dateFilter = '';
    const params = [tenant.id];

    if (period === 'weekly') {
      dateFilter = " AND cl.created_at >= datetime('now', '-7 days')";
    } else if (period === 'monthly') {
      dateFilter = " AND cl.created_at >= datetime('now', '-30 days')";
    }
    // period === 'all' or undefined → no date filter (cumulative)
    ...
    rankings = db.prepare(`
      ...
      FROM students s
      JOIN classes c ON s.class_id = c.id
      LEFT JOIN coin_logs cl ON cl.student_id = s.id${dateFilter}
      WHERE s.tenant_id = ?
      GROUP BY s.id
